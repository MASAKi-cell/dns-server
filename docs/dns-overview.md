...
応答サイズが512バイトを超える場合、権威サーバーは応答のヘッダに**TCビット（Truncated bit）**を立てて「応答が切り詰められた」ことを示します。クライアントはこのTCビットを検出すると、同じクエリをTCPで再送信し、完全な応答を取得します。
...
ゾーンとは、DNSの名前空間において、ある管理者が権限を持って管理する範囲のことです。ドメインとゾーンは似ていますが、異なる概念です。
...
DNSは設計当初、セキュリティをあまり考慮していませんでした。そのため、いくつかの脆弱性や攻撃手法が存在します。

### キャッシュポイズニング

**キャッシュポイズニング（Cache Poisoning）**は、フルリゾルバのキャッシュに偽のDNS情報を注入する攻撃です。

#### 攻撃の仕組み

1. 攻撃者はフルリゾルバに対して、特定のドメインの名前解決を行わせます
2. フルリゾルバが権威サーバーからの応答を待っている間に、攻撃者が偽の応答を大量に送信します
3. 偽の応答がタイミングよく受け入れられると、フルリゾルバのキャッシュに偽の情報が保存されます
4. 以降、そのドメインへのアクセスは攻撃者が指定した偽のIPアドレスに誘導されます

#### 対策

- **ソースポートランダム化**: クエリの送信元ポートをランダムにすることで、攻撃者が正しいポートを推測することを困難にします
- **Query ID（トランザクションID）のランダム化**: 16ビットのQuery IDをランダムにし、偽応答の成功率を下げます
- **DNSSEC**: 後述するDNSSECにより、応答の真正性を検証できます

### DNS増幅攻撃（DNS Amplification Attack）

DNS増幅攻撃は、DNSを悪用したDDoS攻撃の一種です。

#### 攻撃の仕組み

1. 攻撃者は送信元IPアドレスを**被害者のIPアドレスに偽装**（IPスプーフィング）します
2. 多数のオープンリゾルバに対して、大きな応答を返すクエリ（例: ANY、TXT）を送信します
3. 各リゾルバは被害者のIPアドレス宛に応答を返します
4. 小さなクエリで大きな応答を生成できるため、トラフィックが**増幅**されます

例えば、60バイトのクエリで3000バイトの応答が返る場合、50倍の増幅率となります。

#### 対策

- **BCP38/BCP84**: ネットワーク境界でIPスプーフィングをブロックします
- **レートリミット（RRL: Response Rate Limiting）**: 権威サーバーが同一IPへの過剰な応答を制限します
- **オープンリゾルバの排除**: 不要な再帰クエリを受け付けないよう設定します

### DNSSEC

**DNSSEC（DNS Security Extensions）**は、DNS応答に電子署名を付加することで、データの真正性と完全性を検証する仕組みです。

#### 基本的な仕組み

1. 権威サーバーは、各リソースレコードに対して秘密鍵で署名を作成します（RRSIGレコード）
2. 公開鍵はDNSKEYレコードとして公開されます
3. 親ゾーンは子ゾーンの公開鍵のハッシュをDSレコードとして保持します
4. フルリゾルバは署名を検証し、データが改ざんされていないことを確認します

```
example.com.    IN  A       93.184.216.34
example.com.    IN  RRSIG   A 13 2 86400 20240301000000 20240201000000 12345 example.com. (署名データ)
example.com.    IN  DNSKEY  257 3 13 (公開鍵データ)
```

#### 信頼の連鎖（Chain of Trust）

DNSSECの信頼は、ルートゾーンから各ゾーンへと連鎖します。
...
フルリゾルバは、ルートゾーンの公開鍵（トラストアンカー）を事前に持っており、そこから検証を開始します。各階層でDSレコードとDNSKEYレコードを照合することで、最終的な応答の真正性を検証できます。

## 今後のトピック

以下のトピックについては、今後のフェーズで詳しく解説する予定です。

| トピック | 対応フェーズ | 内容 |
|---------|-------------|------|
| TCPフォールバック | 4 | TCビット、512バイト超の応答 |
| EDNS0 | 6 | 拡張DNS、大きなUDPペイロード |
| DNSSEC（詳細実装） | 6 | 署名検証の実装、信頼の連鎖の構築 |
