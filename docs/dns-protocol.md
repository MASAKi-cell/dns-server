...
DNSメッセージは、クエリ・レスポンスを問わず同一フォーマットで、5つのセクションから構成されます(RFC1035 4節)。
...
RFC1035 4.1.1で定義されています。全フィールドを合わせて12byte(96bit)です。
...
| TC | 1bit | TrunCation。UDPの512byte制限等でメッセージが切り詰められたことを示します。立っていればクライアントはTCPで再送すべきです |
...
RFC1035 4.1.2で定義されています。QDCOUNT件分繰り返します。
...
RFC1035 4.1.3で定義されています。Answer/Authority/Additionalの各セクションはこのフォーマットの繰り返しです。
...
RDLENGTHは実データ長から算出する値です。受信側はRDLENGTHぶんだけ読み進めた位置が、そのTYPE用パーサが実際に消費したbyte数と一致することを検証すべきです(不一致はメッセージが壊れているか、パーサにバグがあることを意味します)。

## 5. ドメイン名のエンコーディング

RFC1035 3.1で定義されています。ドメイン名はラベルの列であり、各ラベルは「長さ(1byte) + ラベル本体」の形式で連続してエンコードされ、末尾は長さ0の1byte(ルートラベル)で終端します。
...
RFC1035 4.1.4で定義されています。同じドメイン名がメッセージ中に繰り返し現れる場合(例: QuestionのQNAMEとAnswerのNAME)、2回目以降は全体を書かず、既出の名前(の一部)を指す**ポインタ**で置き換えてメッセージサイズを削減できます。

ポインタは2byteで、先頭2bitが`11`であることで通常のラベル長(先頭2bitは`00`)と区別します。残り14bitがメッセージ先頭からのオフセットを表します。
...
- ポインタは必ず**後方(より小さいオフセット)** を指します。前方参照や自己参照は不正なメッセージとして拒否しなければなりません(無限ループを防ぐため)
- ラベル列の途中からポインタに切り替わってもかまいません(例:`4 'm' 'a' 'i' 'l' <ポインタ>` = "mail." + ポインタ先の名前)
- デコーダはポインタを何回でも追従しうるため、実装は最大追従回数などでループを防御する必要があります

## 7. RDATAのフォーマット(TYPE別)

RFC1035 3.3で定義されています(AAAAのみRFC3596)。
...
RFC1035 3.3で定義されています。TXTレコードなどで使われる、長さ1byteプレフィックス付きの可変長文字列(Pascal文字列形式)です。
「長さ(1byte, 0〜255) + 本体」の形式で、ドメイン名のラベルとは異なり圧縮ポインタの対象にはなりません。
...
- **UDP**: 512byteを超えるメッセージは切り詰められ、HeaderのTCビットが立てられます
  (EDNS0(RFC6891)により実際にはより大きなUDPペイロードが使われることも多いですが、
  本リポジトリでは未対応です)
- **TCP**: メッセージの前に2byteのビッグエンディアン長プレフィックスを付与して送ります
  (RFC1035 4.2.2)。TCビットが立ったレスポンスを受け取ったクライアントはTCPで再送します
...
Additionalの各セクションが、ほぼそのままの構成で出力に現れます。
...
そのままでは名前解決の役に立ちません(いわゆる鶏と卵の問題)。そこでサーバーは、そのNSレコードに
対応するA/AAAAレコードをAdditionalセクションに**グルーレコード**として付加しています。
